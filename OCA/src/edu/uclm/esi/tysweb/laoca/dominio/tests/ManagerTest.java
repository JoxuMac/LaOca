...
public class ManagerTest {
	...
	public void partida3JugadoresGanaPorLlegadaACasilla62() {
		...
		try {
			...
			manager.tirarDado(partida.getId(), a.geteMail(), 1);
			manager.tirarDado(partida.getId(), b.geteMail(), 1);
			manager.tirarDado(partida.getId(), c.geteMail(), 1); // (1, 1, 1)
			
			manager.tirarDado(partida.getId(), a.geteMail(), 4);
			manager.tirarDado(partida.getId(), a.geteMail(), 2);
			manager.tirarDado(partida.getId(), a.geteMail(), 4); // (21, 1, 1)
			assertTrue(a.getCasilla().getPos()==21 && b.getCasilla().getPos()==1 && c.getCasilla().getPos()==1);
			
			manager.tirarDado(partida.getId(), b.geteMail(), 3);
			manager.tirarDado(partida.getId(), b.geteMail(), 6); // (21, 14, 1)
			assertTrue(a.getCasilla().getPos()==21 && b.getCasilla().getPos()==14 && c.getCasilla().getPos()==1);
			
			manager.tirarDado(partida.getId(), c.geteMail(), 1);  // (21, 14, 2)
			assertTrue(a.getCasilla().getPos()==21 && b.getCasilla().getPos()==14 && c.getCasilla().getPos()==2);
			
			manager.tirarDado(partida.getId(), a.geteMail(), 4);  // (52, 14, 2)
			manager.tirarDado(partida.getId(), a.geteMail(), 5);  // (55, 14, 2) // a cae en la muerte, se le elimina
			assertTrue(partida.getJugadores().size()==2);		  // (--, 14, 2)
			
			manager.tirarDado(partida.getId(), b.geteMail(), 1);  // (--, 15, 2)
			assertTrue(b.getCasilla().getPos()==15 && c.getCasilla().getPos()==2);
			
			manager.tirarDado(partida.getId(), c.geteMail(), 3);  // (--, 15, 11)
			manager.tirarDado(partida.getId(), c.geteMail(), 6);  // (--, 15, 22)
			manager.tirarDado(partida.getId(), c.geteMail(), 3);  // (--, 15, 52)
			manager.tirarDado(partida.getId(), c.geteMail(), 4);  // (--, 15, 58)
			
			manager.tirarDado(partida.getId(), b.geteMail(), 2);  // (--, 15, 22)
			manager.tirarDado(partida.getId(), b.geteMail(), 4);
			manager.tirarDado(partida.getId(), b.geteMail(), 4);
			manager.tirarDado(partida.getId(), b.geteMail(), 4);

			manager.tirarDado(partida.getId(), b.geteMail(), 5);

			manager.tirarDado(partida.getId(), c.geteMail(), 6);
	...
	public void partida3JugadoresGanaPorLlegadaACasilla58() {
		...
		try {
			...
			manager.tirarDado(partida.getId(), c.geteMail(), 6);  // (--, 15, 22)
			manager.tirarDado(partida.getId(), c.geteMail(), 3);  // (--, 15, 52)
			manager.tirarDado(partida.getId(), c.geteMail(), 6);  // (--, 15, 58)
			assertSame(partida.getGanador(), c);
		}
		catch (Exception e) {
			fail("No esperaba excepción: " + e.getMessage());
	...
	public void testPartida2Jugadores() {
		...
		try {
			...
			JSONObject mensaje=manager.tirarDado(partida.getId(), a.geteMail(), 3);
			...
			// (3, 0) / turno: b; saca : 3
			mensaje=manager.tirarDado(partida.getId(), b.geteMail(), 3);
			System.out.println(mensaje);
			assertTrue(a.getCasilla().getPos()==3);
			assertTrue(b.getCasilla().getPos()==3);
			
			// (3, 3) / turno: a; saca : 1
			mensaje=manager.tirarDado(partida.getId(), a.geteMail(), 1); // Cae en oca 4
			System.out.println(mensaje);
			assertTrue(a.getCasilla().getPos()==8);
			assertEquals(mensaje.get("mensaje"), Tablero.de_Oca_A_Oca);
			
			// (8, 3) / turno: a; saca : 3
			mensaje=manager.tirarDado(partida.getId(), a.geteMail(), 3);  // Cae en puente 11
			System.out.println(mensaje);
			assertTrue(a.getCasilla().getPos()==5);
			assertEquals(mensaje.get("mensaje"), Tablero.dePuenteAPuente);
			
			// (5, 3) / turno: a; saca : 3
			mensaje=manager.tirarDado(partida.getId(), a.geteMail(), 3);  // Cae en oca 8
			System.out.println(mensaje);
			assertTrue(a.getCasilla().getPos()==13);
			assertEquals(mensaje.get("mensaje"), Tablero.de_Oca_A_Oca);
			
			// (13, 3) / tira a y saca 3, va a la 16
			mensaje=manager.tirarDado(partida.getId(), a.geteMail(), 3);
			System.out.println(mensaje);
			assertTrue(a.getCasilla().getPos()==16);
			
			// (16, 3) / tira b y saca 3
			mensaje=manager.tirarDado(partida.getId(), b.geteMail(), 2); // Cae en puente 5
			System.out.println(mensaje);
			assertTrue(b.getCasilla().getPos()==11);
			
			// (16, 11) / tira b y saca 2
			mensaje=manager.tirarDado(partida.getId(), b.geteMail(), 2); // Cae en oca 13
			System.out.println(mensaje);
			assertTrue(b.getCasilla().getPos()==17);
			
			// (16, 17) / tira b y saca 1
			mensaje=manager.tirarDado(partida.getId(), b.geteMail(), 1); // Cae en posada (3 turnos sin tirar)
			System.out.println(mensaje);
			assertTrue(b.getCasilla().getPos()==18);
			assertTrue(b.getTurnosSinTirar()==3);
			
			// (16, 18) / tira a, b no puede tirar 3 turnos; a saca 5
			mensaje=manager.tirarDado(partida.getId(), a.geteMail(), 5); // Cae en 21
			System.out.println(mensaje);
			...
			mensaje=manager.tirarDado(partida.getId(), a.geteMail(), 4); // Cae en dados 25
			System.out.println(mensaje);
			...
			mensaje=manager.tirarDado(partida.getId(), a.geteMail(), 2); // Cae en 54
			System.out.println(mensaje);
			...
			mensaje=manager.tirarDado(partida.getId(), a.geteMail(), 1); // Cae en 55
			System.out.println(mensaje);
			...
			mensaje=manager.tirarDado(partida.getId(), a.geteMail(), 1); // Cae en 56
			System.out.println(mensaje);
			assertTrue(a.getCasilla().getPos()==56);
			assertSame(partida.getJugadorConElTurno(), b);
			
			// (56, 18) / tira b, saca 4
			mensaje=manager.tirarDado(partida.getId(), b.geteMail(), 4); // Cae en oca 22
			System.out.println(mensaje);
			assertTrue(b.getCasilla().getPos()==26);
			
			// (56, 26) / tira b, saca 5
			mensaje=manager.tirarDado(partida.getId(), b.geteMail(), 5); // Cae en oca 31
			System.out.println(mensaje);
			assertTrue(b.getCasilla().getPos()==35);
			
			// (56, 35) / tira b, saca 5
			mensaje=manager.tirarDado(partida.getId(), b.geteMail(), 5); // Cae en oca 40
			System.out.println(mensaje);
			assertTrue("" + b.getCasilla().getPos(), b.getCasilla().getPos()==44);
			
			// (56, 44) / tira b, saca 5
			mensaje=manager.tirarDado(partida.getId(), b.geteMail(), 5); // Cae en oca 49
			assertTrue(b.getCasilla().getPos()==53);
			
			// (56, 53) / tira b, saca 5
			mensaje=manager.tirarDado(partida.getId(), b.geteMail(), 4); // Cae en la muerte (57)
			System.out.println(mensaje);
	...
	public void testPartidaAleatoria() {
		...
		try {
			...
			do {
				...
				JSONObject mensaje = manager.tirarDado(partida.getId(), turno.geteMail(), dado.nextInt(6)+1);
